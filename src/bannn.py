...
def write(motor, value):
    ...
    if(motor == "servo"):
        ...
        Board.setPWMServoPulse(6, value, 100)
        #print("no motor")
...
if __name__ == '__main__':
    ...
    speed = 1350 #variable for the speed of the car
    ...
    while True:
        ...
        left_contours_top, hierarchy = cv2.findContours(imgThresh[ROI_LEFT_TOP[1]:ROI_LEFT_TOP[3], ROI_LEFT_TOP[0]:ROI_LEFT_TOP[2]],
        cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        right_contours_top, hierarchy = cv2.findContours(imgThresh[ROI_RIGHT_TOP[1]:ROI_RIGHT_TOP[3], ROI_RIGHT_TOP[0]:ROI_RIGHT_TOP[2]],
            cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        left_contours_bot, hierarchy = cv2.findContours(imgThresh[ROI_LEFT_BOT[1]:ROI_LEFT_BOT[3], ROI_LEFT_BOT[0]:ROI_LEFT_BOT[2]],
            cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        right_contours_bot, hierarchy = cv2.findContours(imgThresh[ROI_RIGHT_BOT[1]:ROI_RIGHT_BOT[3], ROI_RIGHT_BOT[0]:ROI_RIGHT_BOT[2]],
            cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
      
        contours_left = left_contours_bot | left_contours_top
        contours_right = right_contours_bot | right_contours_top
