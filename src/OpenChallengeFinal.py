...
# proportion constants for  the servo motor angle
...
picam2.set_controls({"Saturation": 1.4, "Brightness": 0.05}) # adjust brightness and saturation of the image
picam2.start()
...
while True:
    ...
    # find black thresholds and store them in a variable
    ...
    for i in range(len(contours_orange)):
        ...
        cv2.drawContours(im, contours_orange, i, (255, 255, 0), 1) # draw the contours for debug utility
    ...
    for i in range(len(contours_blue)):
        ...
        cnt[:, :, 0] += ROI4[0]  # Add X offset
        cnt[:, :, 1] += ROI4[1]  # Add Y offset
        cv2.drawContours(im, contours_blue, i, (255, 255, 0), 1) # draw the contours for debug utility
        
        
    if track_dir == "none": # find which direction the track is going
        if max_blue_area > max_orange_area and max_blue_area > LINE_THRESHOLD: # if blue is first seen, the track direction is left (counter clockwise)
            track_dir = "left"
        if max_orange_area > max_blue_area and max_orange_area > LINE_THRESHOLD: # if orange is first seen, the track direction is right (clockwise)
            track_dir = "right"
            
    if track_dir == "right": 
        if (
            turn_dir == "right" # if the robot is in a sharp turn but sees the opposite colour, the turn is stopped
        ...
        if (
            turn_dir == "left" # if the robot is in a sharp turn but sees the opposite colour, the turn is stopped
            and max_orange_area > LINE_THRESHOLD
            and max_blue_area < LINE_THRESHOLD
        ):
            done_turning = True
    ...
    if done_turning: # if the robot has just completed a turn, make it continue the turn to assure that it sees the two parallel walls
        turn_length_counter += 1

        if turn_length_counter > TURN_ITER_LIMIT: # turn for the required amount of time
            turn_length_counter = 0
            done_turning = False
        ...
        if turn_dir == "right": # calculate the servo angle for the current turn
            ...
            if left_area < WALL_THRESHOLD: # if the left area is below the threshold, set the servo angle to max for a sharp left turn
                servo_angle = MID_SERVO + MAX_TURN_DEGREE
            elif right_area < WALL_THRESHOLD: # if the right area is below the threshold, set the servo angle to max for a sharp right turn
                servo_angle = MID_SERVO - MAX_TURN_DEGREE
            else:
    ...
    if turn_counter == MAX_TURNS and not done_turning: #if the total turns has surpassed the amount required, increment the action counter by 1
